...
const CATTR = { v_position: 0, v_dir: 1, v_na: 2, v_nb: 3 };
const contourShader = link(CONTOUR_VS, LINE_FS, CATTR);
...
if (HAS_CONTOURS) {
  ...
  contourVao = gl.createVertexArray();
  gl.bindVertexArray(contourVao);
  gl.bindBuffer(gl.ARRAY_BUFFER, cvbo);
  gl.enableVertexAttribArray(CATTR.v_position);
  gl.vertexAttribPointer(CATTR.v_position, 3, gl.FLOAT, false, 48, 0);
  gl.enableVertexAttribArray(CATTR.v_dir);
  gl.vertexAttribPointer(CATTR.v_dir,      3, gl.FLOAT, false, 48, 12);
  gl.enableVertexAttribArray(CATTR.v_na);
  gl.vertexAttribPointer(CATTR.v_na,       3, gl.FLOAT, false, 48, 24);
  gl.enableVertexAttribArray(CATTR.v_nb);
  gl.vertexAttribPointer(CATTR.v_nb,       3, gl.FLOAT, false, 48, 36);
